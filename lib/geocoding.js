...
if (!GOOGLE_API_KEY) throw new Error(`Google API Key not set. Run with -a flag!`)
...
async function geocodeAddress (address) {
  const geocodeData = await googleMapsClient.geocode({address})
  if (geocodeData.status !== 200) throw new Error(geocodeData.json.error_message)
...
function normalizeAddress (place, addressString) {
  if (!place) return Object.assign({addressString}, normalizedAddress)
  const addressStringParts = addressString.split(',').map(a => a.trim()) 
  const address = Object.assign({}, normalizedAddress, {
